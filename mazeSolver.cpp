...
vector<CPoint> mazeSolver::m_neighbors{{1,0},{0,1},{-1,0},{0,-1}};  //Initializes a static vector m_neighbors with four points
//representing the possible moves in the maze (up, down, left, right).

mazeSolver::mazeSolver(const size_t rows,const size_t cols, const QGraphicsView *view, const int startX, const int startY, const int endX, const int endY,const int editX,const int editY,QString type)
...
void mazeSolver::generate_maze()
{
    for(int i = 0; i < m_rows - 1; ++i)
    {
        for(int j = 0; j < m_cols - 1; ++j)
        {
            ...
            /*if(m_edit.x()==i && m_edit.y()==j)
            {
                qDebug() << "the point is found"<<i<<j;
                if(m_type=="Wall")
                {
                    m_maze[i][j]='#';
                    std::cout<<"since the type is wall the we will put #";
                }
                else{
                    m_maze[i][j]='0';
                    std::cout<<"since the type is not wall then we will put 0";
                }
            }*/
        }
    }
    ...
    std::cout<< "the type is newnew form the func";
    if(m_type=="Wall")
    {
        m_maze[m_edit.x()][m_edit.y()]='#';
        std::cout<<"since the type is wall the we will put #";
        std::cout << std::endl;
    }
    else if(m_type=="Space")
    {
        m_maze[m_edit.x()][m_edit.y()]='0';
        std::cout<<"since the type is not wall then we will put 0";
        std::cout << std::endl;
    }
    else
    {
        std::cout<<"somthing wrong happened";
    }
    std::cout << std::endl;
    std::cout <<"hanoooon" <<m_maze[m_edit.x()][m_edit.y()] << " ";
    std::cout << std::endl;
}

void mazeSolver::print_maze() const{
    std::cout<<"output the contents of the maze to the console";
    std::cout << std::endl;
...
void mazeSolver::display_maze(QGraphicsView* view){
    ...
    for (int i = 0; i < m_rows; i++) {
        for (int j = 0; j < m_cols; j++) {
            ...
            {
                m_items[i][j]->setBrush(QBrush(Qt::yellow));
                std::cout<<"yelowwwwwwww";
            }
...
void mazeSolver::resetMaze() {
    ...
    m_maze.resize(m_rows, vector<char>(m_cols, '0')); // Resize the maze to the correct dimensions
    m_visited.clear(); // Clear the visited data structure
    m_visited.resize(m_rows, vector<bool>(m_cols, false)); // Resize the visited data structure to the correct dimensions
    m_parent.clear(); // Clear the parent data structure
    m_parent.resize(m_rows, vector<CPoint>(m_cols)); // Resize the parent data structure to the correct dimensions
    m_items.clear();
    m_items.resize(m_rows,vector<QGraphicsRectItem*>(m_cols));
}
